...
float calc_shadow_factor() {
    float bias = 0.01;
	float depth = texture(_shadow_map, fragShadowCoords.xy).r + bias;
...
void main() {
    ...
    vec3 diffuse = /* _material.color.xyz * */ diff * _light.color.xyz * shadow_factor;

    vec3 result = clamp(diffuse, 0.0, 1.0);
